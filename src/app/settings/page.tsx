...
export default function SettingsPage() {
  ...
  const [settingsTab, setSettingsTab] = useState<
    "teacher" | "api" | "models" | "options" | "backup"
  >("teacher");
    ...
    >
      <div className="space-y-4">
        ...
        <Card title="새 모델 등록">
          ...
          <div className="mt-3">
            <button type="button" className={btnPrimary} onClick={onAddModel}>
              모델 등록
            </button>
          </div>
          ...
          <div className="mt-4 space-y-2">
            ...
            {customModels.length === 0 ? (
              <p className="text-sm text-slate-500">등록된 사용자 모델이 없습니다.</p>
            ) : (
                ...
                {customModels.map((m) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      value={m.modelId}
                      onChange={(e) =>
                        updateCustomModel(m.id, { modelId: e.target.value })
                      }
                      placeholder="모델 ID"
                    />
                    <button
        ...
        <Card title="AI API 키 등록">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field
              label="모델"
              hint="목록에서 고르거나, 직접 입력해도 됩니다."
            >
              <select
                className={`${inputClass} mb-2`}
                value={
                  modelOptions.some((m) => m.modelId === form.model)
                    ? form.model
                    : "__custom__"
                }
                onChange={(e) => {
              ...
              >
                {modelOptions.map((m) => (
                  <option key={`${m.source}-${m.modelId}`} value={m.modelId}>
                    {m.label} ({m.modelId})
                    {m.source === "custom" ? " · 등록" : ""}
                  </option>
                ))}
              ...
              <datalist id={`models-${form.provider}`}>
                {modelOptions.map((m) => (
                  <option key={`dl-${m.modelId}`} value={m.modelId}>
                    {m.label}
                  </option>
                ))}
              </datalist>
          ...
          {data.settings.apiKeys.length === 0 ? (
            ...
            <ul className="space-y-3">
              {data.settings.apiKeys.map((key) => {
                const options = listModelsForProvider(key.provider, customModels);
                return (
                  <li
                    key={key.id}
                    ...
                    <div className="mt-2 grid gap-2 sm:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        <select
                          className={inputClass}
                          value={
                            options.some((m) => m.modelId === key.model)
                              ? key.model
                              : "__custom__"
                          }
                          onChange={(e) => {
                            if (e.target.value === "__custom__") return;
                            updateApiKey(key.id, { model: e.target.value });
                          }}
                        >
                          {options.map((m) => (
                            <option
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            >
              <select
                ...
                onChange={(e) =>
                  setTeacherSubjectId(e.target.value ? e.target.value : null)
                }
              >
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setTeacherClassName(e.target.value ? e.target.value : null)
                }
                placeholder="예: 2-3"
              />
              <datalist id="teacher-class-options">
                {[
                  ...new Set(
                    data.students.map((s) => s.className).filter(Boolean),
                  ),
                ]
        ...
        <Card title="생성 옵션">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {(Object.keys(SECTION_LABELS) as Section[]).map((section) => (
              <Field key={section} label={`${SECTION_LABELS[section]} 글자 수`}>
                <input
                  className={inputClass}
