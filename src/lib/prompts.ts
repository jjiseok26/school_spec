...
export function buildSystemPrompt(section: Section, charLimit: number | null) {
    ...
    ? [
        ...
        ...(section === "autonomy"
          ? [
              ...
              "- 임원 활동과 일반 일정 활동이 함께 주어지면 임원 내용을 별도 문장으로 먼저 쓰고, 일정 활동 문장과 섞지 않는다.",
  ...
  return [
    ...
    "- 서술어는 탁월함, 뛰어남, 확장함, 성장함, 분석함, 추론함, 토론함, 발휘함, 적극적임, 돋보임, 우수함, 제시함, 기여함, 이끌어냄, 설명함, 공유함, 발표함 같은 표현을 위주로 사용한다.",
    '- 작성 중 반드시 "~함", "~음", "~임", "~하였음"과 같은 명사형(개조식) 종결어미를 사용한다.',
...
export function buildUserPrompt(input: GenerationInput) {
    ...
    parts.push(
      "[수합 지시] 아래는 이미 작성된 특기사항 초안이다. 중복을 줄이고 흐름이 자연스럽도록 하나의 특기사항으로 수합하라. 각 초안에 없는 사실은 추가하지 않는다." +
        (isActivitySection(input.section)
          ...
          : "") +
        (input.section === "autonomy"
          ? " 임원 관련 초안과 일정 활동 초안이 함께 있으면 임원 내용을 앞쪽에, 일정 활동 내용을 뒤쪽에 두고 문장을 서로 섞지 마라."
          : ""),
    );
  }
  ...
  if (activityMode) {
    if (hasOfficers && !hasActivities) {
      parts.push(
        [
          "위 임원 활동만 뼈대로 하고, 교사 관찰·학생 문서·상담 자료를 근거로 임원 특기사항만 작성하라.",
          "일정 활동 문장은 쓰지 마라. 임원은 「임원명(시작일-종료일)」 형식만 쓰고 «N학년:» 표기는 금지한다.",
          "최상/상/중/하 등급 초안 4개를 JSON으로 작성하라.",
        ].join(" "),
      );
    } else if (hasActivities && !hasOfficers) {
      parts.push(
        [
          "위 체크된 활동만 뼈대로 하고, 각 항목의 교사 관찰·학생 문서·상담 자료를 근거로 특기사항을 작성하라.",
          "임원 활동 문장은 쓰지 마라. 일반 활동은 「활동명(YYYY.MM.DD.)에서 …함.」 형식을 따르며, 최상/상/중/하 등급 초안 4개를 JSON으로 작성하라.",
        ].join(" "),
      );
    } else {
      parts.push(
        [
          "임원 활동과 체크된 일정을 각각 별도 문장으로 작성하라. 임원 내용을 먼저 쓰고, 이어서 일정 활동을 쓰며 문장을 서로 섞지 마라.",
          "임원은 「임원명(시작일-종료일)」 형식만 쓰고 «N학년:» 표기는 금지한다. 일반 활동은 「활동명(YYYY.MM.DD.)에서 …함.」 형식을 따른다.",
          "최상/상/중/하 등급 초안 4개를 JSON으로 작성하라.",
        ].join(" "),
      );
    }
